...
// ── Gamma Levels Persistence ──────────────────────────────────
const GAMMA_FILE = path.join(__dirname, 'gamma_levels.json');

function loadGamma() {
  try {
    if (fs.existsSync(GAMMA_FILE)) {
      const raw = fs.readFileSync(GAMMA_FILE, 'utf8');
      return JSON.parse(raw);
    }
  } catch (e) {
    console.warn('Could not load gamma_levels.json:', e.message);
  }
  return {};
}

function saveGamma(data) {
  try {
    fs.writeFileSync(GAMMA_FILE, JSON.stringify(data, null, 2), 'utf8');
  } catch (e) {
    console.error('Could not write gamma_levels.json:', e.message);
  }
}

// Load persisted data on startup
let alerts = loadAlerts();
let gammaLevels = loadGamma();
console.log(`Loaded ${alerts.length} persisted alert(s) from disk`);
console.log(`Loaded gamma levels for ${Object.keys(gammaLevels).length} ticker(s) from disk`);
...
// ── Gamma Levels API ──────────────────────────────────────────
app.get('/gamma-levels', (req, res) => {
  res.json(gammaLevels);
});

app.post('/gamma-levels', (req, res) => {
  try {
    const body = typeof req.body === 'string' ? JSON.parse(req.body) : req.body;
    gammaLevels = body;
    saveGamma(gammaLevels);
    res.json({ ok: true });
  } catch (e) {
    res.status(400).json({ ok: false, error: e.message });
  }
...
app.delete('/gamma-levels', (req, res) => {
  gammaLevels = {};
  saveGamma(gammaLevels);
  res.json({ ok: true });
});
